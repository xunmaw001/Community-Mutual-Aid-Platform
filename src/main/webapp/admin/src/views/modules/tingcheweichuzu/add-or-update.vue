...
export default {
  ...
  methods: {
	...
	this.ruleForm.neirongxiangqing = this.ruleForm.neirongxiangqing.replace(reg,'../../../ssm016gm/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `tingcheweichuzu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.tingcheweichuzuCrossAddOrUpdateFlag = false;
						 this.parent.search();
